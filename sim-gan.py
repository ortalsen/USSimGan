...
from my_dlutils import plot_image_batch_w_labels
...
def refiner_network(input_image_tensor):
    """
    The refiner network, Rtheta, is a residual network (ResNet). It modifies the synthetic image on a pixel level, rather
...
def discriminator_network(input_image_tensor):
    """
    The discriminator network, Dphy, contains 5 convolution layers and 2 max-pooling layers.
...
def adversarial_training(low_level_images_dir, goal_images_dir, refiner_model_path=None, discriminator_model_path=None):
    """Adversarial training of refiner network Rtheta and discriminator network Dphy."""
    #
    ...
    synthetic_generator = datagen.flow_from_directory(
        directory=low_level_images_dir,
        **flow_from_directory_params
    )

    real_generator = datagen.flow_from_directory(
        directory=goal_images_dir,
        **flow_from_directory_params
    )
    ...
    if not refiner_model_path:
        # we first train the Rtheta network with just self-regularization loss for 1,000 steps
    ...
    if not discriminator_model_path:
        # and Dphy for 200 steps (one mini-batch for refined images, another for real)
    ...
    for i in range(nb_steps):
        ...
        for _ in range(k_g * 2):
            ...
            # update theta by taking an SGD step on mini-batch loss LR(theta)
        ...
        for _ in range(k_d):
            ...
            # update phy by taking an SGD step on mini-batch loss LD(phy)
...
def main(low_level_images_dir, goal_images_dir, refiner_model_path, discriminator_model_path):
    adversarial_training(low_level_images_dir, goal_images_dir, refiner_model_path, discriminator_model_path)
